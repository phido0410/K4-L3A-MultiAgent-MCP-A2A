...
Mở `.env` và điền **Team API Key** Phi gửi riêng (dạng `sk-team-...`):
...
> **Lưu ý:** `pytest -q` hiện có **1 test FAIL** (`test_release_safety.py`). Đó là test dành cho repo gốc lúc phát hành, nó assert `inputs/` phải rỗng. Không phải lỗi của bạn, Phi xử lý. Các test còn lại phải xanh.

Nếu `day09 mcp-tools` lỗi auth → nhắn Phi, đừng tự sửa key.
...
Thêm: **không đoán tên tool MCP**. Luôn chạy `day09 mcp-tools` để biết tool nào tồn tại.
...
Mỗi file có **đúng một chủ**. Không sửa file của người khác; cần đổi thì nhắn chủ file hoặc mở PR.

```text
src/student_agent/
├── workflow.py              [PHI]   coordinator
├── a2a.py                   [PHI]   giao tiếp giữa agent
├── domain/
│   ├── findings.py          [PHI]   kiểu dữ liệu chung  ← hai bạn code DỰA VÀO file này
│   ├── rules.py             [PHI]   ghép tín hiệu → primary_issue
│   └── money.py             [QUỐC]  tính tiền hoàn
└── agents/
    ├── order_agent.py       [BẢO]
    ├── shipment_agent.py    [BẢO]
    ├── payment_agent.py     [QUỐC]
    ├── policy_agent.py      [QUỐC]
    └── verifier.py          [PHI]
```
...
```python
async def analyze(case, gateway, trace) -> Finding:
    case_id = case["case_id"]

    # 1. Gọi MCP — case_id được gateway tự nhét vào, KHÔNG tự thêm
    evidence = await gateway.call("<tên_tool_thật>", case_id=case_id, order_id=oid)

    ref  = evidence["evidence_ref"]   # lấy nguyên văn
    data = evidence["data"]
    warns = evidence.get("warnings", [])   # đọc warnings, có thể là tín hiệu quan trọng

    # 2. Suy luận từ data → sinh signals + facts
    signals, facts = [], {}
    ...

    # 3. Ghi trace CHO MỖI evidence thực sự dùng để kết luận
    trace.emit(
        case_id=case_id,
        event_type="tool_result_consumed",
        actor="order-agent",
        tool_name="<tên_tool_thật>",
        evidence_refs=[ref],
    )

    return Finding(actor="order-agent", signals=signals, facts=facts, ...)
```

**Ba lỗi hay gặp:**

- Gọi MCP rồi quên `trace.emit(...)` → mất điểm `evidence` và `workflow`.
- Cite ref không thực sự dùng để kết luận → điểm `evidence` là **F1**, cite thừa bị phạt y như cite thiếu. Gọi 10 tool nhưng chỉ 3 tool dẫn tới kết luận thì chỉ cite 3.
- Truyền tay `case_id` vào `arguments` → `gateway.call()` đã tự thêm rồi, truyền tay dễ gây sai scope.
...
### B1 — Khảo sát tool (làm TRƯỚC, D1–D2)

```bash
day09 mcp-tools
```

Chọn 3–5 case mẫu, gọi thử từng tool thuộc domain `order`, `item`, `seller`, `shipment`, `product`. Ghi lại **tên field thực tế** trong `data` vào `docs/domain-order-shipment.md`.

> ⚠️ Mục B4/B5 dưới đây dựa trên **giả định** field giống dataset Olist gốc. Phải đối chiếu với output thật trước khi code cứng. Nếu tên field khác, báo cả nhóm ngay.
...
### Q1 — Khảo sát tool (làm TRƯỚC, D1–D2)

```bash
day09 mcp-tools
```

Gọi thử tool thuộc domain `payment`, `refund`, `policy`, `customer` trên 3–5 case. Ghi field thực tế vào `docs/domain-payment-policy.md`.

> ⚠️ Các mục dưới giả định field giống Olist gốc (`payment_sequential`, `payment_type`, `payment_installments`, `payment_value`). **Verify trước khi code cứng.**
