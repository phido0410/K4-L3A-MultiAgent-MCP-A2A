...
| 2026-09-25 | Phi | Khám nghiệm hai bài nộp 0 điểm | 97/100 case rỗng evidence và trùng luồng trace. Khớp với chẩn đoán "spam" của giảng viên |
| 2026-09-25 | Phi | Thêm `check_evidence_health` và `check_trace_health` chặn đóng gói; thiếu `h2` thành lỗi chết | Đã kiểm trên chính bộ output hỏng, cả hai cửa đều bắt |
| | | _TODO_ | |
...
| R-13 | **Hai bài nộp 0 điểm (12:06, 12:10).** Giảng viên: trace bị trùng, hệ thống gắn cờ spam. Khám nghiệm khớp: 97/100 case có luồng trace giống hệt nhau, 0 evidence_ref, 11 tool call cho cả batch | ✅ **Đã thêm cửa chặn 2026-09-25** | Nguyên nhân gốc: kết nối MCP chết giữa run, agent bắt `except Exception` nên lỗi vô hình — `day09 validate` vẫn pass. `day09 package` giờ chặn khi >15% case rỗng evidence hoặc >40% case trùng luồng trace | Phi |
| R-12 | **Hai người cùng chạy lệnh gọi MCP trên cùng Team API Key.** Contract yêu cầu `evidence_ref` khớp **team, run VÀ case**; sai thì dính hard gate `cross_scope_evidence_ref` → case 0 điểm | **Rất cao — có thể mất toàn bộ bài nộp** | Chỉ **một người** được gọi MCP tại một thời điểm. Áp dụng cho cả `day09 run`, `day09 mcp-tools`, `scripts/smoke_cases.py`, `scripts/dump_case.py`. Báo group trước khi bắt đầu và sau khi xong. Nếu nghi có người chạy chồng, chạy lại sạch một mình trước khi nộp | Cả nhóm |
